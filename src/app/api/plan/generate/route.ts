...
import { validatePlanProgression, validateSessionVariety, generateCoachIntro } from '@/lib/plan-validation';
...
export async function POST() {
  ...
  // ── POST-GENERATION: Progression validation (Bug 2) ──
  const progressionCorrections = validatePlanProgression(plan);
  for (const correction of progressionCorrections) {
    console.warn(`[PlanGenerate] PROGRESSION CLAMP: Week ${correction.weekNumber}: ${correction.originalKm}km → ${correction.correctedKm}km (${correction.reason})`);
  }
  if (progressionCorrections.length === 0) {
    console.log('[PlanGenerate] Progression validation passed — no weekly volume corrections needed');
  }

  // ── POST-GENERATION: Session variety validation (Bug 3) ──
  const varietyFailures = validateSessionVariety(plan);
  for (const failure of varietyFailures) {
    console.warn(`[PlanGenerate] VARIETY FAIL: ${failure.reason}`);
  }
  if (varietyFailures.length === 0) {
    console.log('[PlanGenerate] Session variety validation passed');
  }

  // ── Session type distribution log ──
  ...
  console.log(`[PlanGenerate] Weekly km baseline: ${weeklyKm}km | Week 1 total: ${plan[0]?.totalDistanceKm || 0}km`);
  ...
  // ── Generate clean, user-facing coach intro (Bug 1) ──
  // This is the ONLY text shown to the user. Internal generation rules are never exposed.
  const coachIntro = generateCoachIntro({
    raceName,
    totalWeeks,
    runsPerWeek,
    thresholdPace: formatPace(thresholdPace),
    activityClause,
  ...
  // Save coach intro as a coach message — NOT the raw generation prompt
